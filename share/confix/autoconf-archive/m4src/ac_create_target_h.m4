...
dnl
dnl if the HEADER-FILE is absent, [target.h] is used. if the PREFIX is
dnl absent, [TARGET] is used. the prefix can be the packagename.
dnl (y:a-z-:A-Z_:)
dnl
...
dnl
dnl NOTE: CREATE_TARGET_H does also fill HOST_OS-defines functionality
dnl has been split over functions called CREATE_TARGET_H_UPPER
dnl CREATE_TARGET_H_LOWER CREATE_TARGET_HOST_UPPER
dnl CREATE_TARGET_HOST_LOWER CREATE_TARGET_H uses CREATE_TARGET_H_UPPER
dnl CREATE_TARGET_HOST_UPPER CREATE_TARGET_H_ uses
dnl CREATE_TARGET_H_LOWER CREATE_TARGET_HOST_LOWER
dnl
