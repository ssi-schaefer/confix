...
dnl
dnl Given a TYPE of the form "int##_t" or "uint##_t", see if the
dnl datatype TYPE is predefined. If not, then define TYPE -- both with
dnl AC_DEFINE and as a shell variable -- to the first datatype of
dnl exactly ## bits in a list of CANDIDATE-TYPEs. If none of the
dnl CANDIDATE-TYPEs contains exactly ## bits, then set the TYPE shell
dnl variable to "no".
dnl
...
dnl datatypes defined by AC_DEFINE_INTEGER_BITS are visible to your
dnl program:
dnl
