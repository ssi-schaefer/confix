...
dnl
dnl AX_LIB_MYSQL macro takes only one argument which is optional. If
dnl there is no required version passed, then macro does not run
dnl version test.
dnl
...
dnl
dnl path - complete path to mysql_config utility, use this option if
dnl mysql_config can't be found in the PATH
dnl
