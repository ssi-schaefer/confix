...
dnl
dnl Try all the combinations of <TAG1>, <TAG2>... to successfully
dnl compile <code>. <TAG1>, <TAG2>, ... are substituted in <code> and
dnl <include> with values found in <values1>, <values2>, ...
dnl respectively. <values1>, <values2>, ... contain a list of possible
dnl values for each corresponding tag and all combinations are tested.
dnl When AC_TRY_COMPILE(include, code) is successfull for a given
dnl substitution, the macro stops and defines the following macros:
dnl FUNCTION_TAG1, FUNCTION_TAG2, ... using AC_DEFINE() with values set
dnl to the current values of <TAG1>, <TAG2>, ... If no combination is
dnl successfull the configure script is aborted with a message.
dnl
dnl Intended purpose is to find which combination of argument types is
dnl acceptable for a given function <function>. It is recommended to
dnl list the most specific types first. For instance ARG1, [size_t,
dnl int] instead of ARG1, [int, size_t].
dnl
...
dnl      ...
dnl
dnl Implementation notes: generating all possible permutations of the
dnl arguments is not easily done with the usual mixture of shell and
dnl m4, that is why this macro is almost 100% m4 code. It generates
dnl long but simple to read code.
dnl
dnl @category Misc
