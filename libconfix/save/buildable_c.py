# $Id: buildable_c.py,v 1.31 2006/03/22 15:03:54 jfasch Exp $
...
# NOTE: this is essentially a copy of buildable_cxx.py, and should
...
from buildinfo_common import BuildInfo_CFLAGS
from helper_configure_in import AC_PROG_CC
from core.error import Error

class BuildableCCreator(BuildableCreator):

    def create_from_file(self, dir, filename, lines):
        if self.is_valid_ext(filename):
            return BuildableC(dir=dir,
                              filename=filename,
                              lines=lines)
        return None
...
class BuildableC(BuildableCBase):
    ...
    def __init__(self,
        ...
        # C compiler flags I get distributed from the content objects
    ...
    def gather_configure_in(self):
        ...
        return AC_PROG_CC + BuildableCBase.gather_configure_in(self)

    def contribute_makefile_am(self, buildmod):

        BuildableCBase.contribute_makefile_am(self, buildmod=buildmod)

        for f in self.cflags_:
            buildmod.makefile_am().add_am_cflags(f)            
